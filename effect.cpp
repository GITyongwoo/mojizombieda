...
static ID3D11ShaderResourceView* g_Texture[TEXTURE_MAX] = { NULL };	// テクスチャ
		...
		{									// Yes
			...
			for (int n = 0; n < effectWk[i].effectCount; n++)
			{
				if (effectWk[i].pParticle[n].isFinish == 0) {
					//バレットの位置やテクスチャー座標を反映
					float px = effectWk[i].pParticle[n].pos.x - enemy[i].pos.x;	// エフェクトの表示位置X
					float py = effectWk[i].pParticle[n].pos.y - enemy[i].pos.y;	// エフェクトの表示位置Y
